...
**Total: 44 file — 26 diedit, 10 file baru (termasuk avatar PNG), 8 dihapus (+3342 / −3040 baris, di luar dokumen ini).**

| File | Status | Ringkasan |
|---|---|---|
| `app.js` | diedit | CSRF, cascade delete stream/history/thumbnail, thumbnail original 2MB, parsing schedule dual-mode (ISO absolut utk picker baru / datetime-local lama utk client lama), `sameSite`, hapus route donators, route hapus semua history, limit login 5→10 percobaan/15 menit, session 24 jam → 7 hari, adopsi stream saat boot (pengganti reset offline), **route publik `/home` `/privacy` `/terms`**, **guard channel disconnected di PUT/POST stream YouTube**, **shutdown dipaksa exit <1 detik + putus koneksi idle** (jaga-jaga agar pm2 tak pernah eskalasi ke tree-kill karena `server.close()` menggantung di socket keep-alive), **fitur signup publik dihapus total** (route GET/POST + view — pembuatan user cukup lewat admin di User Management; `/setup-account` utk admin pertama TETAP ada), **12 route mati dihapus** (0 referensi frontend, sudah diverifikasi menyeluruh): `/welcome-bypass`, `POST /api/users/status` + `/role` (tergantikan modal edit), `/api/settings/youtube-status` (tergantikan render server + `/youtube-channels`), 3 route API key Google Drive (import Drive jalan tanpa API key), `/api/stream/videos` (legacy), `POST /upload/video` (legacy), 3 route resume chunk upload (`status`/`pause`/`delete` — UI-nya gak pernah dibuat) — total route 100 → 88, **alur welcome dihapus** (route `/welcome` + `/welcome/continue`, redirect setup-account langsung ke `/dashboard` — jadi 86 route), **catch-all 404** (path tak dikenal → halaman error branded dgn status 404, bukan teks polos "Cannot GET /xxx" bawaan Express; path `/api/*` tak dikenal balas JSON biar fetch frontend gak error parse), **fix bug upstream thumbnail audio** (referensi `.png` yg gak pernah ada → `.svg` yg ada), **avatar default user baru** nunjuk `/images/default-avatar.png` (dulu ke path uploads yg gak pernah dibuat), **fix route shadowing `GET /api/streams/check-key`** (sebelumnya tertimpa `:id`), **fallback route gambar warisan** (`default-channel.png`, `default-thumbnail.jpg`, `video-placeholder.png` dialihkan ke asset SVG/PNG yang valid), **fix bug upstream `encrypt is not defined`** (fungsi `utils/encryption` dipanggil di ~8 titik tapi gak pernah di-import top-level — nyimpen credential YouTube selalu crash `ReferenceError`; kini di-import sekali di atas, 2 require lokal redundan dihapus) |
| `db/database.js` | diedit | Tambah index `idx_stream_history_stream_id` + kolom `streams.ffmpeg_pid` + kolom `youtube_channels.is_connected`; hapus migrasi kolom `users.welcome_shown` (ikut alur welcome yg dihapus) |
...
| `public/images/logo_mobile.svg` | **dihapus** | Idem |
| `public/images/default-avatar.jpg` | **dihapus** | Diganti `default-avatar.png` |
| `public/images/onedrive.png` | **dihapus** | Import OneDrive gak pernah ada (modal cloud cuma Drive/Mediafire/Dropbox/Mega) |
| `public/images/saweria.svg` | **dihapus** | UI donasi udah dihapus sebelumnya |
| `public/js/chunkedUploader.js` | **dihapus** | Implementasi chunk upload lama — yang kepake versi inline di gallery.ejs; sekaligus 3 endpoint resume-nya juga dihapus dari app.js |
...
| `views/gallery.ejs` | diedit | Definisi dialog duplikat diganti file shared; modal media player (JS) + modal import cloud ikut fitur tutup-via-backdrop; **layout mobile: tombol Upload Media & Import from Cloud full width jadi 2 baris, search full width, filter type + sort sebaris 50/50** |
| `views/history.ejs` | diedit | Dialog custom + tombol Delete All + truncate judul panjang biar tabel gak scroll horizontal + waktu dirender ulang di sisi browser (timezone lokal pengunjung); lebar filter platform w-32 → w-44 (biar "All Platforms" muat); fallback thumbnail video `onerror` diarahkan ke SVG yang valid; **layout mobile: search full width, filter platform + sort sebaris 50/50** |
| `views/layout.ejs` | diedit | Meta CSRF + csrf.js + custom-dialog.js, pembersihan UI (notifikasi/donate/footer), avatar pindah ke header, **rebrand dashboard "Kumix Stream"** (title suffix, logo sidebar desktop+mobile, favicon `kumix-logo.png`), **sidebar dirombak jadi lebar (w-60 / 240px)**: logo + nama brand "Kumix Stream" sebaris di header sidebar, menu icon + label sebaris (dulu rail 80px icon di atas teks + tooltip), **menu Settings pindah ke sidebar (di bawah Users, ikut highlight halaman aktif), dihapus dari dropdown avatar header**, **mode mobile dirombak**: bottom nav + popup profile mobile dihapus, sidebar yang sama jadi drawer slide-in lewat tombol hamburger di header (di samping avatar; tutup via backdrop/Escape/klik menu), avatar muncul di header mobile (dropdown profile sama persis dgn desktop) — semua menu (termasuk History/Users/Settings yg dulu gak ada di bottom nav) kini tersedia di mobile, **logo + brand di header (mobile & desktop) jadi link ke `/dashboard`**, **sidebar desktop z-index `lg:z-30` + container `min-w-0`** (mencegah tabel lebar membocorkan scroll horizontal ke level window dan menumpuk di atas sidebar), **retheme merah Kumix** (tailwind config `primary #E8102A` / `secondary #C00D22`; config sama diterapkan di semua halaman standalone + publik) |
| `views/error.ejs` | diedit | Rebrand "Kumix Stream" (title, logo, favicon); header kini logo + nama brand "Kumix Stream" |
| `views/login.ejs` | diedit | Rebrand "Kumix Stream" (title, logo, favicon), link "Sign up" dihapus; header kini logo + nama brand "Kumix Stream" |
| `views/signup.ejs` | **dihapus** | Fitur signup publik dihilangkan — user baru dibuat admin lewat User Management |
| `views/setup-account.ejs` | diedit | Rebrand "Kumix Stream" (title, logo, favicon, teks intro) |
| `views/playlist.ejs` | diedit | Dialog hapus playlist pakai dialog custom; tombol **Select all** di Available Videos/Audio (nambah semua yang tampil di list, ikut filter search — sekali klik buat banyak data) + tombol **Reshuffle** di Selected Videos/Background Music (acak urutan, Fisher-Yates); toast info di-retheme ke `bg-primary`; fallback thumbnail video `onerror` diarahkan ke SVG yang valid; **default Video Playback = Shuffle**; **layout mobile: search & filter sort full width (ditumpuk)** |
| `views/rotations.ejs` | diedit | Dialog hapus rotation pakai dialog custom; teks badge tag YouTube `text-blue-300` → `text-primary`; fix infinite loop `onerror` thumbnail channel & video |
| `views/settings.ejs` | diedit | 4 dialog konfirmasi (reCAPTCHA, logs, disconnect channel) pakai dialog custom; channel disconnected tetap tampil dengan badge + tombol Reconnect; **tab About dihapus** (tombol, panel, loader kontributor GitHub); **retheme ke primary merah**: tab/badge/tombol + alert info reCAPTCHA (bg/border/ikon/teks — tadinya biru; level log INFO & link konsol Google sengaja tetap biru, semantik); **select "Last N lines" di-fix**: w-40 + `sm:inline-flex` (display block/inline-block pada custom select bikin teks + icon arrow jadi 2 baris — UA base-select memakai inline-flex); **layout mobile**: tab Settings 50% (grid 2 baris) + toolbar Logs (search fullwidth, select line & tombol sejajar) |
| `views/users.ejs` | diedit | Modal konfirmasi statis lama diganti dialog shared; **filter "Live Now"** (user yg lagi ada stream live — pake `activeStreamCount` yg udah dihitung route, client-side via `data-live`); modal edit/create/video/stream dikasih class `modal-overlay` (ikut fitur tutup-via-backdrop); spinner & badge ukuran video di-retheme ke primary merah; **layout mobile: search full width, filter roles + status sebaris 50/50, filter streaming full width** |
| `views/welcome.ejs` | **dihapus** | Alur welcome dihilangkan total (halaman sekali-pakai setelah setup-account): route `/welcome` + `/welcome/continue` dihapus, redirect setup-account langsung ke `/dashboard`, migrasi kolom `welcome_shown` dibuang |

---

## Ringkasan Perilaku Baru

- **Tema warna Kumix**: seluruh elemen tema berubah dari biru StreamFlow ke merah Kumix `#E8102A` (diambil persis dari logo) dengan hover/secondary `#C00D22` — mencakup tailwind config di 8 halaman, CSS var `styles.css`, 32 hover tombol, tab Settings + alert reCAPTCHA, theme `info` dialog custom, toggle class di stream-modal, dan hardcode bottom-nav mobile. Elemen **semantik** tetap biru by design: toast/dialog INFO (level log), ikon Facebook, stat upload. Versi cache SW dinaikkan (1.8.1) biar client dapet asset baru.
...
12. **Stream scheduled yang macet gak loop selamanya** — kalau start gagal 5x beruntun (dalam window 1 jam), stream otomatis di-parkir `offline` dengan alasan kegagalannya tercatat di console log stream-nya (sebelumnya: dicoba ulang tiap 15 detik tanpa batas, membanjiri log dan memukul API). **Cancel schedule maupun parkir-gagal hanya mengubah status** — jam mulai & selesai tetap tersimpan, jadi buka form edit langsung terisi jadwal lama dan reschedule tinggal simpan ulang (dulu: semua waktu terhapus saat cancel). **Start manual otomatis membuang sisa jadwal lama** — tanpa ini, `end_time` basi dari jadwal yang di-cancel bisa menghentikan stream beberapa detik setelah mulai.
13. **Penjadwalan timezone-safe end-to-end + stop end-time presisi** — semua form schedule (manual RTMP maupun YouTube) kini submit **ISO absolut** dan server menyimpan momen persisnya, jadi jam jadwal gak bergeser walau timezone browser dan server beda (client lama/API script yang masih kirim `datetime-local` tetap diterima, dibaca sebagai waktu server seperti dulu). Tampilan jadwal di dashboard memakai waktu lokal browser si user, dan filter tanggal menghitung tanggal lokal server (bukan UTC) sehingga stream dini hari gak nyasar ke tanggal sebelumnya. Stream yang punya end-time kini di-stop **tepat pada waktunya** via timer per-stream (dulu hanya di-sweep tiap 30 detik, bisa telat sampai ~30 detik).
14. **Pengerasan sisa + halaman publik** — (a) edit yang mengosongkan jadwal gak pernah menyentuh status stream **live** lagi (dulu bisa flip offline sesaat via API); (b) menghapus stream ikut menghapus file log ffmpeg-nya (`logs/ffmpeg/stream-<id>.log`) serta jejak memorinya (console buffer, counter kegagalan schedule, timer pending); (c) channel **disconnected** gak bisa dipilih lagi di picker create/edit (di-disable + tooltip) dan ditolak server dengan pesan jelas; (d) halaman History menampilkan waktu dalam timezone lokal pengunjung; (e) tersedia halaman publik **`/home`, `/privacy`, `/terms`** (tanpa login) untuk diisi ke Google Cloud Console → OAuth consent screen sebelum app di-PUBLISH.
