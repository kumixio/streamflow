...
| `public/sw.js` | diedit | Static resources jadi network-first, precache + csrf.js + custom-dialog.js, versi cache 1.8.0 (precache logo ganti `kumix-logo.png`) |
...
| `views/history.ejs` | diedit | Dialog custom + tombol Delete All + truncate judul panjang biar tabel gak scroll horizontal + waktu dirender ulang di sisi browser (timezone lokal pengunjung); lebar filter platform w-32 → w-44 (biar "All Platforms" muat); **layout mobile: search full width, filter platform + sort sebaris 50/50** |
| `views/layout.ejs` | diedit | Meta CSRF + csrf.js + custom-dialog.js, pembersihan UI (notifikasi/donate/footer), avatar pindah ke header, **rebrand dashboard "Kumix Stream"** (title suffix, logo sidebar desktop+mobile, favicon `kumix-logo.png`), **sidebar dirombak jadi lebar (w-60 / 240px)**: logo + nama brand "Kumix Stream" sebaris di header sidebar, menu icon + label sebaris (dulu rail 80px icon di atas teks + tooltip), **menu Settings pindah ke sidebar (di bawah Users, ikut highlight halaman aktif), dihapus dari dropdown avatar header**, **mode mobile dirombak**: bottom nav + popup profile mobile dihapus, sidebar yang sama jadi drawer slide-in lewat tombol hamburger di header (di samping avatar; tutup via backdrop/Escape/klik menu), avatar muncul di header mobile (dropdown profile sama persis dgn desktop) — semua menu (termasuk History/Users/Settings yg dulu gak ada di bottom nav) kini tersedia di mobile, **logo + brand di header (mobile & desktop) jadi link ke `/dashboard`**, **retheme merah Kumix** (tailwind config `primary #E8102A` / `secondary #C00D22`; config sama diterapkan di semua halaman standalone + publik) |
...
| `views/playlist.ejs` | diedit | Dialog hapus playlist pakai dialog custom; tombol **Select all** di Available Videos/Audio (nambah semua yang tampil di list, ikut filter search — sekali klik buat banyak data) + tombol **Reshuffle** di Selected Videos/Background Music (acak urutan, Fisher-Yates); toast info di-retheme ke `bg-primary`; **default Video Playback = Shuffle**; **layout mobile: search & filter sort full width (ditumpuk)** |
| `views/rotations.ejs` | diedit | Dialog hapus rotation pakai dialog custom; teks badge tag YouTube `text-blue-300` → `text-primary` |
